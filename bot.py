...
class Bot(Client):
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL_REQUEST_1:
            try:
                link = (await self.get_chat(FORCE_SUB_CHANNEL_REQUEST_1, creates_join_request = True)).invite_link
                if not link:
        ...
        if FORCE_SUB_CHANNEL_REQUEST_2:
            try:
                link = (await self.get_chat(FORCE_SUB_CHANNEL_REQUEST_2, creates_join_request = True)).invite_link
                if not link:
        ...
        if FORCE_SUB_CHANNEL_REQUEST_3:
            try:
                link = (await self.get_chat(FORCE_SUB_CHANNEL_REQUEST_3, creates_join_request = True)).invite_link
                if not link:
