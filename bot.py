...
from config import API_HASH, APP_ID, LOGGER, TG_BOT_TOKEN, TG_BOT_WORKERS, FORCE_SUB_CHANNEL_1, FORCE_SUB_CHANNEL_2, FORCE_SUB_CHANNEL_3, FORCE_SUB_CHANNEL_4, FORCE_SUB_CHANNEL_REQUEST_1, FORCE_SUB_CHANNEL_REQUEST_2, FORCE_SUB_CHANNEL_REQUEST_3, FORCE_SUB_CHANNEL_REQUEST_4, CHANNEL_ID
...
class Bot(Client):
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL_REQUEST_4:
            try:
                link = (await self.create_chat_invite_link(FORCE_SUB_CHANNEL_REQUEST_4, creates_join_request = True)).invite_link
                if not link:
                    await self.create_chat_invite_link(FORCE_SUB_CHANNEL_REQUEST_4, creates_join_request = True)
                    link = (await self.create_chat_invite_link(FORCE_SUB_CHANNEL_REQUEST_4, creates_join_request = True)).invite_link
                self.reinvitelink4 = link
            except Exception as a:
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning("Bot can't Create Invite link from Fourth Force Sub Channel!")
                self.LOGGER(__name__).warning(f"Please Double check the FORCE_SUB_CHANNEL_REQUEST_4 value and Make sure Bot is Admin in channel with Invite Users via Link Permission, Current First Force Sub Channel Value: {FORCE_SUB_CHANNEL_REQUEST_4}")
                self.LOGGER(__name__).info("\nBot Stopped. Join https://t.me/CodeXBotzSupport for support")
                sys.exit()
