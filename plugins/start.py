...
REPLY_PERROR = """📢 اطلاع‌رسانی(غیرقابل ذخیره سازی)\n\nروی پیام مورد نظر ریپلای نمائید و مجدد <code>/pbroadcast</code> را ارسال کنید."""

REPLYPERROR = """📢 فوروارد پیام(غیرقابل ذخیره سازی)\n\nروی پیام مورد نظر ریپلای نمائید و مجدد <code>/pforward</code> را ارسال کنید."""
...
‎◌ /pbroadcast ↴
  ᴥ ارسال پیام(غیرقابل ذخیره سازی) به کاربران ربات

‎◌ /forward ↴
  ᴥ فوروارد پیام به کاربران ربات

‎◌ /pforward ↴
  ᴥ فوروارد پیام(غیرقابل ذخیره سازی) به کاربران ربات
...
async def protect_send_text(client: Bot, message: Message):
    if message.reply_to_message:
        query = await query_msg()
        broadcast_msg = message.reply_to_message
        total = 0
        ...
        pls_wait = await message.reply("📢 پیام اطلاع‌رسانی(غیرقابل ذخیره سازی)\nاین عمل ممکن است کمی طول بکشد ...")
        for row in query:
            chat_id = int(row[0])
            try:
        ...
        status = f"""✅ اطلاع‌رسانی(غیرقابل ذخیره سازی) کامل شد
...
• ناموفق: <code>{unsuccessful}</code></b>"""
        ...
        msg = await message.reply(REPLY_PERROR)
        await asyncio.sleep(8)
        await msg.delete()

@Bot.on_message(filters.private & filters.command('pforward') & filters.user(ADMINS))
async def protect_forward_message(client: Bot, message: Message):
    if message.reply_to_message:
        query = await query_msg()
        broadcast_msg = message.reply_to_message
        total = 0
        ...
        pls_wait = await message.reply("📢 فوروارد پیام(غیرقابل ذخیره سازی)\nاین عمل ممکن است کمی طول بکشد ...")
        for row in query:
            chat_id = int(row[0])
            try:
                await broadcast_msg.forward(chat_id, protect_content = True)
                successful += 1
            except FloodWait as e:
                await asyncio.sleep(e.x)
                await broadcast_msg.forward(chat_id)
        ...
        status = f"""✅ فوروارد پیام(غیرقابل ذخیره سازی) کامل شد
...
• ناموفق: <code>{unsuccessful}</code></b>"""
        ...
        msg = await message.reply(REPLYPERROR)
        await asyncio.sleep(8)
        await msg.delete()
