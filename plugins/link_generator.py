...
async def batch(client: Client, message: Message):
    ...
    string = f"a-{(f_msg_id * int(str(abs(client.db_channel.id))[8:]) + X_NUM)}-{(s_msg_id * int(str(abs(client.db_channel.id))[8:]) + X_NUM)}"
...
async def link_generator(client: Client, message: Message):
    ...
    base64_string = await encode(f"a-{(msg_id * int(str(abs(client.db_channel.id))[6:]) + X_NUM)}")
    link = f"t.me/{client.username}?start={base64_string}"
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("• اشتراک گذاری لینک", url=f'https://telegram.me/share/url?url={link}')]])
