...
async def batch(client: Client, message: Message):
    ...
    string = f"{(f_msg_id * int(str(abs(client.db_channel.id))[3:])) + STATIC_NUM}-{(s_msg_id * int(str(abs(client.db_channel.id))[3:])) + STATIC_NUM}"
...
async def link_generator(client: Client, message: Message):
    ...
    base64_string = await encode(f"{(msg_id * int(str(abs(client.db_channel.id))[3:])) + STATIC_NUM}")
    link = f"https://t.me/{client.username}?start={base64_string}"
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("• اشتراک گذاری لینک", url=f'https://telegram.me/share/url?url={link}')]])
